...
const collegeDetails = async function (req, res) {
  try {
    ...
    let interns = await internModel.find({ collegeId: id }).select({_id:1, name:1, email:1, mobile:1, email:1});

    if (interns.length == 0) {
      let noIntern = "This College Doesn't Have Any Intern"
      interns = noIntern;
    }
    const colleges = { name, fullName, logoLink, interns }

    return res.status(200).send({ status: true, msg: "List Of The Interns Of This College", data: { colleges } })
  }
